...
{
    int type = 1;       // 1 for Put, otherwise for Call
    double scale = 5.0; // Factor for Call Price: Smax = scale*BS::K
    
	double sig = 0.4;
	double K = 120.0;
	double T = 1.5;
	double r = 0.04;
    double b = 0.04; // Added by Chako
	double D = 0.0;  // aka q
...
int main()
{
    ...
    BS::type = -1;   // 1 for Put; otherwise for Call
    BS::scale = 5;  // Smax = scale*BS::K
    
    int J = 5 * BS::K; int N = 100-1;
    
    double Smax = BS::scale * BS::K; // Magix, scale determines call price calculation
    ...
    cout << "S, FDM solution, Exact solution" << endl;
    for (int i = 0; i < fdir.xarr.Size(); i++)
    {
        if (BS::type == 1)
        {
            cout << fdir.xarr[i+1] << ", " << fdir.current()[i+1] << ", " << S_PutPrices2[i] << endl;
        }
        else
        {
            cout << fdir.xarr[i+1] << ", " << fdir.current()[i+1] << ", " << S_CallPrices2[i] << endl;
        }
    }
